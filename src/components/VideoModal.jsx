...
import { X, Play, Pause, Check, Download, Layers, ShieldCheck, Clock, Calendar, Tag, Monitor, Smartphone, Square, Maximize } from 'lucide-react';
...
export default function VideoModal({ video, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
      <div
        className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto text-neutral-100 shadow-2xl relative flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header Bar */}
        <div className="sticky top-0 bg-[#111]/95 backdrop-blur border-b border-white/10 p-4 px-6 flex items-center justify-between z-20">
          <div className="flex items-center gap-3">
            <span className="text-[#e63946] font-bold text-xs uppercase tracking-widest bg-[#e63946]/10 px-2.5 py-1 rounded">
              {video.category}
            </span>
            {video.exclusive && (
              <span className="text-yellow-400 font-bold text-xs bg-yellow-400/10 px-2 py-0.5 rounded border border-yellow-400/30">
                ⭐ EXCLUSIVE
              </span>
            )}
          </div>
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="relative rounded-xl overflow-hidden bg-neutral-900 border border-white/10 aspect-video flex flex-col justify-between p-4 group">
            {/* Watermark overlay */}
            {watermarkOn && (
              <div className="absolute top-4 right-4 bg-black/60 backdrop-blur border border-white/20 px-3 py-1 rounded text-xs font-black tracking-widest text-[#e63946]">
                REDWIRE PREVIEW
              </div>
            )}
            ...
            <div className="flex-1 flex items-center justify-center relative">
              <button
                ...
                className="w-16 h-16 rounded-full bg-[#e63946] text-white flex items-center justify-center shadow-[0_0_30px_rgba(230,57,70,0.6)] hover:scale-110 transition-transform cursor-pointer"
              >
            ...
            {/* Custom Control Bar */}
            <div className="bg-black/70 backdrop-blur-md rounded-lg p-3 flex flex-col gap-2 border border-white/10">
              <div
          ...
          {/* Metadata Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-neutral-900/60 border border-white/5 rounded-xl p-4 text-xs">
            <div>
            ...
            <div>
              <span className="text-neutral-500 block mb-1">Formats</span>
              <span className="font-semibold text-neutral-200 flex items-center gap-1.5">
                <Monitor size={13} className="text-[#e63946]" /> {video.formats.join(', ')}
              </span>
            </div>
            <div>
              <span className="text-neutral-500 block mb-1">Timestamps</span>
              <span className="font-semibold text-neutral-200 flex items-center gap-1.5">
          ...
          <div>
            ...
            <p className="text-neutral-300 text-sm leading-relaxed">{video.desc}</p>
          </div>

          {/* Timestamps */}
          <div>
            <h3 className="text-xs uppercase font-bold tracking-wider text-neutral-400 mb-3">Shot List & Timestamps</h3>
            <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-1">
              {video.timestamps.map((ts, i) => (
                <div
                  key={i}
                  onClick={() => handleSeek(ts.t)}
                  className="flex items-center gap-4 p-2.5 rounded-lg bg-neutral-900/40 hover:bg-[#e63946]/10 border border-white/5 hover:border-[#e63946]/30 cursor-pointer transition-colors"
                >
                  <span className="text-xs font-mono font-bold text-[#e63946] bg-[#e63946]/10 px-2 py-1 rounded">
                    {ts.t}
                  </span>
                  <span className="text-xs text-neutral-300">{ts.d}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-neutral-900 border border-white/10 rounded-xl p-5 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="text-xs text-neutral-400 block mb-2 font-medium">Resolution</label>
              ...
              <div>
                <label className="text-xs text-neutral-400 block mb-2 font-medium">Aspect Ratio</label>
                <div className="flex flex-wrap gap-1.5">
              ...
              <div>
                <label className="text-xs text-neutral-400 block mb-2 font-medium">Format</label>
                <div className="flex flex-wrap gap-1.5">
                  {['MP4', 'MOV', 'MXF'].map((fmt) => (
                    <button
                      key={fmt}
            ...
            <div className="flex items-center justify-between pt-3 border-t border-white/10">
              <div>
                <span className="text-xs font-semibold text-white block">Overlay Watermark</span>
                <span className="text-[11px] text-neutral-400">Embed REDWIRE branding on exported footage</span>
              </div>
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                  downloadSuccess
                    ...
                    : 'bg-[#e63946] hover:bg-[#ff4757] text-white shadow-[0_4px_20px_rgba(230,57,70,0.4)]'
                }`}
              >
                {downloading ? (
